		...
		if (!hitVector[i])
		{
				...
				ball->getY() < currentBlock->getY() + currentBlock->getHeight())
			{
				...
				*Vertices = { 0.0f, 0.0f, 0.0f, currentBlock->getColor() };
				...
				float distBottom = abs(ball->getY() - (currentBlock->getY() + currentBlock->getHeight()));
				float distTop = abs(ball->getY() + ball->getHeight() - currentBlock->getY());
				float distLeft = abs(ball->getX() + ball->getWidth() - currentBlock->getX());
				float distRight = abs(ball->getX() - (currentBlock->getX() + currentBlock->getWidth()));

				float distH = min(distLeft, distRight);
				float distV = min(distTop, distBottom);

				//distH += (ball->getVelocityX());
				//distV += (ball->getVelocityY());
